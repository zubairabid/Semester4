...
\font\bg="FreeSerif:script=beng" at 9pt
...
\begin{document}
	...
	\Tree[.STORY
					...
					[.PLOT
								[.EPISODE
											...
											[.OUT-\\COME ] ] ]
					[.RESOLUTION
								[.EVENT\\and\\STATE Mary\\not\\caught
													Police\\ate\\evidence ] ] ]\\	
	$ $\\
	$ $\\
	$ $\\
	$ $\\
	\Tree[.CHARACTERS {Mary\\Maloney} {Patrick\\Maloney} 
			{Sam} {Jack\\Noonan} {O' Malley} ]
	$ $\\
	$ $\\
	$ $\\
	$ $\\
	...
	\item \Tree[.EPISODE
					[.SUBGOAL {Talk to}\\{husband when}\\{he gets home} ]
					[.EVENT {Mary}\\{waited for}\\{her}\\{husband} 
					{She}\\{made drinks}\\{for both}\\{and made}\\{some small}\\talk 
					Patrick\\{spoke to}\\Mary\\about\\leaving\\her 
					{Mary got}\\{up in a daze}\\{to make}\\{supper}
					{Mary picked}\\{up a frozen}\\{leg of lamb} ]
					[.OUTCOME {Mary walking to}\\{kitchen with a}\\{frozen
					lamb leg}\\{while disraught} ] ]
%	$ $\\
%	$ $\\
		
	\item \Tree[.EPISODE
						[.SUBGOAL Prepare\\{an alibi}\\{after killing}\\Patrick ]
						[.EVENT {Patrick says}\\{he'll eat}\\{out, Mary}\\{clubs him}\\{to
						death}\\{with leg}\\{of lamb} {Mary thinks}\\{about
						justice}\\{system and}\\{decides not}\\{to risk it} 
						{Practices}\\{asking grocer}\\{normally for}\\{food 
						and}\\{talks to}\\{him, acting}\\{normal}
						{Prepares to}\\{fake emotional}\\{response}\\{for calling}\\{police
						about}\\{the crime}\\{and calls them}
						{On police}\\{coming,}\\{gives them}\\story\\{of
						her}\\{going to}\\{the grocers}\\{and finding}\\{him
						dead}\\{on arrival} ]
						[.OUTCOME {The police}\\{check her}\\{alibi
						with}\\{Sam the}\\{grocer and}\\{conclude 
						it}\\{was not}\\{her because}\\{of 
						strong}\\{alibi and}\\{normal}\\{behaviour} ] ]

	\item \Tree [.EPISODE
						[.SUBGOAL {Hide the}\\{murder}\\{weapon} ]
						[.EVENT {After killing}\\{Patrick, she}\\{put
						the}\\{lamb leg}\\{in the}\\{oven}
						{She was}\\{comforted}\\{by police}\\{and Jack}\\{Noonan
						gave}\\{her some}\\{details and}\\{their}\\{suspicions}
						{Police were}\\{getting tired}\\{so she}\\{offered}\\{them
						a}\\{drink, and}\\{Jack found}\\{the lamb}\\{cooking}
						{Mary offered}\\{the leg of}\\{lamb 
						(the}\\{murder weapon)}\\{to the}\\{police
						for}\\{dinner and}\\{they ate}\\{it} ]
						[.OUTCOME {The murder}\\{weapon was}\\{eaten} ] ]	
